...
def aes_encrypt_content(content, encrypt_key, charset='utf-8'):
    length = len(bytes(content, encoding=charset))
    padded_content = pad(content, length)
    iv = '\0' * BLOCK_SIZE
    cryptor = AES.new(encrypt_key.encode(charset), AES.MODE_ECB, iv)
...
def aes_decrypt_content(encrypted_content, encrypt_key, charset='utf-8'):
    encrypted_content = base64.b64decode(encrypted_content)
    iv = '\0' * BLOCK_SIZE
    cryptor = AES.new(encrypt_key.encode('utf-8'), AES.MODE_CBC, iv)
