...
from tools.cerberus import errors
from tools.cerberus.platform import _int_types, _str_type
from tools.cerberus.schema import (schema_registry, rules_set_registry,
                             DefinitionSchema, SchemaError)
from tools.cerberus.utils import (drop_item_from_tuple, isclass,
...
class BareValidator(object):
    ...
    def _error(self, *args):
        ...
        if len(args) == 1:
            ...
            if self.error_message:
                error_message_info = self.error_message.get(field)
                error_message = error_message_info.get(rule) if error_message_info else ""
            self.recent_error = errors.ValidationError(
                document_path, schema_path, code, rule, constraint, value, info, error_message
            )
    ...
    def __init_processing(self, document, schema=None, error_message=None):
        self._errors = errors.ErrorList()
    ...
    def validate(self, document, schema=None, update=False, normalize=True, error_message=None):
        ...
        self.__init_processing(document, schema, error_message)
        if normalize:
            self.__normalize_mapping(self.document, self.schema)
    ...
    def _validate_remark(self, definitions, field, value):
        """ {'nullable': False }  """
        pass
    ...
    def __validate_required_fields(self, document, error_message=None):
